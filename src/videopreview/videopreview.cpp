...
#include "videopreview/videopreviewconfigdialog.h"
...
bool VideoPreview::extractImages() {
	...
	VideoInfo i = getInfo(prop.input_video);
	int length = i.length;
...
bool VideoPreview::runPlayer(int seek, double aspect_ratio) {
	...
	if (Settings::pref->isMPV()) {
		...
		args << "--no-config" << "--no-audio" << "--no-cache"
			 << "--frames=6" << "--framedrop=no"
			 << "--start=" + QString::number(seek);
		if (aspect_ratio != 0) {
		...
		if (prop.extract_format == PNG) {
			args << "-vo" << "png:outdir=\""+full_output_dir+"\"";
		} else {
	...
	// Log command
	QString command = Settings::pref->playerAbsolutePath();
	for (int n = 0; n < args.count(); n++)
		command = command + " " + args[n];
	qDebug("VideoPreview::runMplayer: command: %s", command.toUtf8().constData());

	QProcess p;
	p.start(Settings::pref->playerAbsolutePath(), args);
	if (!p.waitForFinished()) {
		qDebug("VideoPreview::runPlayer: error running process");
		error_message = tr("The player process didn't run");
		return false;
	}
...
void VideoPreview::cleanDir(const QString& directory) {
...
VideoInfo VideoPreview::getInfo(const QString& filename) {
	...
	if (Settings::pref->isMPV()) {
		...
		args << "--vo=null" << "-ao=null" << "--frames=1"
			 << "--no-quiet" << "--no-cache" << "--no-config";
